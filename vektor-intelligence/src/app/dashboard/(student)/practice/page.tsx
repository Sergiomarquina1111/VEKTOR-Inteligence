...
// ── Mock question bank — replace with /api/practice in Session 8 ──
const MOCK_QUESTIONS: Question[] = [
  ...
  },
  {
    ...
    correct: "b",
    explanation: "If 0 is an eigenvalue, then det(A − 0·I) = det(A) = 0, so A is singular and therefore not invertible.",
  },
  {
  ...
  },
  {
    ...
    explanation: "Symmetry ensures Aᵀ = A, which forces all eigenvalues to be real and guarantees eigenvectors for distinct eigenvalues are orthogonal.",
  },
  {
    ...
    correct: "b",
    explanation: "Entangled particles share a quantum state. Measuring one instantly determines the correlated state of the other, regardless of distance.",
  },
  {
    ...
    correct: "c",
    explanation: "The gradient points in the direction of steepest ascent. To minimize, we move opposite to the gradient — toward the minimum.",
  },
  {
    ...
    correct: "b",
    explanation: "Linear dependence means at least one vector can be expressed as a linear combination of the others.",
  },
];

// ── Real: count today's practice sessions from Firestore ───────────
async function fetchUsedToday(userId: string): Promise<number> {
  try {
    ...
    // If collection doesn't exist yet, return 0
    return 0;
  }
}
...
async function mockScore(answer: string, question: Question): Promise<AnswerResult> {
  ...
  return {
    score,
    correct: score >= 0.7,
    verdict: score >= 0.7 ? "Good understanding demonstrated."
           : score >= 0.4 ? "Partially correct — key concepts missing."
...
// ── Question Card ──────────────────────────────────────────────────
function QuestionCard({ question, onAnswer, isPro, questionNumber, total }: {
  question: Question; onAnswer: (result: AnswerResult) => void;
  isPro: boolean; questionNumber: number; total: number;
}) {
  ...
  async function handleSubmit() {
    ...
    if (!isFree) {
      ...
      res = { score: correct ? 1 : 0, correct, verdict: correct ? "Correct!" : "Incorrect.", correction: correct ? "" : question.explanation };
    } else {
      setScoring(true);
    ...
    setResult(res);
    setTimeout(() => { setFlipped(true); setTimeout(() => onAnswer(res), 600); }, res.correct ? 400 : 800);
  }
      ...
      <div className="transition-all duration-500"
        ...
        <div className="p-6 flex flex-col gap-5"
          style={{
            ...
            backfaceVisibility: "hidden", transition: "border-color 0.3s",
          }}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="flex items-center justify-center flex-shrink-0 text-sm font-mono"
                style={{ width: 32, height: 32, border: `1px solid ${color}40`, backgroundColor: `${color}10`, color, fontFamily: "var(--font-dm-mono)" }}
              >{symbol}</div>
              <div>
                <div className="text-xs font-black" style={{ color, fontFamily: "var(--font-syne)" }}>{label}</div>
                <div className="text-xs" style={{ color: "#3A3A5C", fontFamily: "var(--font-dm-mono)" }}>{question.concept}</div>
              </div>
            </div>
            <div className="flex items-center gap-4">
              <DifficultyPips level={question.difficulty} color={color} />
              <span className="text-xs" style={{ color: "#3A3A5C", fontFamily: "var(--font-dm-mono)" }}>{formatTime(elapsed)}</span>
              <span className="text-xs" style={{ color: "#3A3A5C", fontFamily: "var(--font-dm-mono)" }}>{questionNumber}/{total}</span>
            </div>
          </div>

          <p className="text-sm leading-relaxed" style={{ color: "#F0F0FF", fontFamily: "var(--font-instrument)", lineHeight: 1.8 }}>
            {question.text}
          </p>
          ...
          {locked ? (
            <div className="flex flex-col items-center gap-3 py-6"
              ...
              <Link href="/settings" className="px-4 py-2 text-xs font-black"
              ...
              {question.options?.map(opt => {
                ...
                return (
                  <button key={opt.id} onClick={() => !submitted && setSelected(opt.id)}
                    ...
                    style={{
                      border: `1px solid ${isCorrect ? "#C8FF0060" : isWrong ? "#FF3D5760" : isSelected ? `${color}60` : "#1E1E36"}`,
                    ...
                    <span className="flex-shrink-0 w-6 h-6 flex items-center justify-center text-xs font-black"
                      style={{ border: `1px solid ${isCorrect ? "#C8FF0060" : isWrong ? "#FF3D5760" : "#1E1E36"}`,
                               color: isCorrect ? "#C8FF00" : isWrong ? "#FF3D57" : "#3A3A5C", fontFamily: "var(--font-dm-mono)" }}
            ...
            <div>
              <textarea value={freeText} onChange={e => setFreeText(e.target.value)}
                disabled={submitted} placeholder="Write your explanation here..."
                rows={5}
                className="w-full p-4 text-sm outline-none resize-none transition-all duration-200 disabled:opacity-70"
                style={{ backgroundColor: "#16162A", border: `1px solid ${freeText ? `${color}40` : "#1E1E36"}`,
                         color: "#F0F0FF", fontFamily: "var(--font-instrument)", lineHeight: 1.7 }}
              />
              <div className="flex items-center justify-between mt-2">
                <span className="text-xs" style={{ color: "#3A3A5C", fontFamily: "var(--font-dm-mono)" }}>
                  {freeText.trim().split(/\s+/).filter(Boolean).length} words
                </span>
                <span className="text-xs" style={{ color: "#3A3A5C", fontFamily: "var(--font-dm-mono)" }}>
                  AI scored · Level {question.difficulty}
                </span>
              </div>
            </div>
          )}
          ...
          {scoring && (
            <div className="flex items-center gap-2">
              <div className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: "#7B5CFF", animation: "pulse 1s infinite" }} />
              <span className="text-xs" style={{ color: "#7B5CFF", fontFamily: "var(--font-dm-mono)" }}>AI scoring your answer...</span>
            </div>
          )}
          ...
          {!submitted && !locked && (
            <button onClick={handleSubmit}
              ...
              style={{ backgroundColor: color, color: "#08080F", fontFamily: "var(--font-syne)" }}
          ...
          {submitted && result && !flipped && (
            <div className="px-4 py-3 flex items-center gap-3"
              style={{ backgroundColor: result.correct ? "#C8FF0010" : "#FF3D5710",
                       border: `1px solid ${result.correct ? "#C8FF0030" : "#FF3D5730"}` }}>
              <span style={{ color: result.correct ? "#C8FF00" : "#FF3D57", fontSize: 16 }}>
                {result.correct ? "✓" : "✗"}
              </span>
              <span className="text-xs" style={{ color: result.correct ? "#C8FF00" : "#FF3D57", fontFamily: "var(--font-instrument)" }}>
                {result.verdict}
              </span>
            </div>
          )}
        ...
        <div className="absolute inset-0 p-6 flex flex-col gap-4"
          style={{ backgroundColor: "#0F0F1A", border: `1px solid ${result?.correct ? "#C8FF0040" : "#FF3D5740"}`,
                   backfaceVisibility: "hidden", transform: "rotateY(180deg)" }}>
          <div className="flex items-center gap-3">
            <span style={{ color: result?.correct ? "#C8FF00" : "#FF3D57", fontSize: 20 }}>
              {result?.correct ? "✓" : "✗"}
            </span>
            <span className="text-sm font-black" style={{ fontFamily: "var(--font-syne)", color: result?.correct ? "#C8FF00" : "#FF3D57" }}>
              {result?.verdict}
            </span>
            {result && !result.correct && (
              <div className="ml-auto px-2 py-1 text-xs"
                style={{ backgroundColor: "#FF3D5715", color: "#FF3D57", border: "1px solid #FF3D5730", fontFamily: "var(--font-dm-mono)" }}>
                Score: {Math.round((result.score || 0) * 100)}%
              </div>
            )}
          </div>
          <div className="p-4" style={{ backgroundColor: "#16162A", border: "1px solid #1E1E36" }}>
            <p className="text-xs mb-2 tracking-widest uppercase" style={{ color: "#3A3A5C", fontFamily: "var(--font-dm-mono)" }}>Explanation</p>
            <p className="text-sm leading-relaxed" style={{ color: "#F0F0FF", fontFamily: "var(--font-instrument)", lineHeight: 1.8 }}>
              {question.explanation}
            </p>
          </div>
          {result && !result.correct && result.correction && (
            <div className="p-4" style={{ backgroundColor: "#7B5CFF10", border: "1px solid #7B5CFF30" }}>
              <p className="text-xs mb-2 tracking-widest uppercase" style={{ color: "#7B5CFF", fontFamily: "var(--font-dm-mono)" }}>Correct answer</p>
              <p className="text-xs leading-relaxed" style={{ color: "#F0F0FF", fontFamily: "var(--font-instrument)", lineHeight: 1.7 }}>
                {result.correction}
              </p>
            </div>
          )}
...
export default function PracticePage() {
  ...
  // ── Real: today's practice session count ──────────────────────
  const [usedToday, setUsedToday] = useState(0);
  useEffect(() => {
    if (!user?.uid) return;
    fetchUsedToday(user.uid).then(setUsedToday);
  }, [user?.uid]);
  ...
  function buildQueue(subj: string, diff: Difficulty): Question[] {
    ...
    return [...filtered].sort((a, b) => Math.abs(a.difficulty - diff) - Math.abs(b.difficulty - diff));
  }

  function handleStart() {
    setQuestions(buildQueue(subject, difficulty));
    setCurrentIndex(0); setResults([]); setConsecutiveRight(0);
    setConsecutiveWrong(0); setFinished(false); setStarted(true);
  }
  ...
  function handleAnswer(result: AnswerResult) {
    ...
    if (result.correct) {
      ...
      setConsecutiveRight(streak); setConsecutiveWrong(0);
      if (streak >= 2 && difficulty < 5) { newDiff = (difficulty + 1) as Difficulty; setDifficulty(newDiff); setConsecutiveRight(0); }
    } else {
      const streak = consecutiveWrong + 1;
      setConsecutiveWrong(streak); setConsecutiveRight(0);
      if (streak >= 2 && difficulty > 1) { newDiff = (difficulty - 1) as Difficulty; setDifficulty(newDiff); setConsecutiveWrong(0); }
    }
  ...
  const avgScore     = results.length > 0 ? Math.round(results.reduce((s, r) => s + r.score, 0) / results.length * 100) : 0;
      ...
      <div className="max-w-2xl mx-auto space-y-8">
        <div>
          <div className="flex items-center gap-3 mb-1">
            ...
            <span className="text-xs tracking-widest uppercase" style={{ color: "#C8FF00", fontFamily: "var(--font-dm-mono)" }}>Practice Mode</span>
          </div>
          <h1 className="text-2xl font-black" style={{ fontFamily: "var(--font-syne)", color: "#F0F0FF" }}>Sharpen your understanding</h1>
        ...
        <div className="p-5" style={{ backgroundColor: "#0F0F1A", border: "1px solid #1E1E36" }}>
          <p className="text-xs mb-4 tracking-widest uppercase" style={{ color: "#3A3A5C", fontFamily: "var(--font-dm-mono)" }}>Choose subject</p>
            ...
            {(user?.subjects || Object.keys(SUBJECT_COLORS)).map(subj => {
              ...
              return (
                <button key={subj} onClick={() => setSubject(subj)}
                  ...
                  style={{ border: `1px solid ${active ? color : "#1E1E36"}`, backgroundColor: active ? `${color}10` : "#16162A" }}>
                  <span style={{ color: active ? color : "#3A3A5C", fontFamily: "var(--font-dm-mono)", fontSize: 14 }}>{SUBJECT_SYMBOLS[subj]}</span>
                  <span className="text-xs font-black" style={{ color: active ? "#F0F0FF" : "#6B6A80", fontFamily: "var(--font-syne)" }}>
                    {SUBJECT_LABELS[subj]}
                  </span>
                </button>
              );
        ...
        <div className="p-5" style={{ backgroundColor: "#0F0F1A", border: "1px solid #1E1E36" }}>
          <div className="flex items-center justify-between mb-4">
            <p className="text-xs tracking-widest uppercase" style={{ color: "#3A3A5C", fontFamily: "var(--font-dm-mono)" }}>Starting difficulty</p>
            <span className="text-xs px-2 py-1"
              style={{ backgroundColor: `${subjectColor}15`, color: subjectColor, border: `1px solid ${subjectColor}30`, fontFamily: "var(--font-dm-mono)" }}>
              Level {difficulty}
            </span>
          </div>
          <div className="flex items-center gap-3">
            ...
            <input type="range" min={1} max={5} value={difficulty}
              ...
              style={{ appearance: "none", height: 3, outline: "none",
          ...
          <div className="flex justify-between mt-2">
            {["Beginner","Elementary","Intermediate","Advanced","Expert"].map((l, i) => (
              <span key={l} className="text-xs"
                style={{ color: i + 1 <= difficulty ? subjectColor : "#1E1E36", fontFamily: "var(--font-dm-mono)", fontSize: 9 }}>
                {l.slice(0, 3)}
              </span>
            ))}
          </div>
        ...
        {/* Free tier notice — shows real usedToday count */}
        {!isPro && (
          <div className="flex items-center justify-between px-4 py-3"
            style={{ backgroundColor: "#FFB80008", border: "1px solid #FFB80025" }}>
            <div className="flex items-center gap-2">
              <span style={{ color: "#FFB800", fontSize: 12 }}>✦</span>
              <span className="text-xs" style={{ color: "#FFB800", fontFamily: "var(--font-dm-mono)" }}>
                Free: {Math.max(0, FREE_DAILY_LIMIT - usedToday)} questions remaining today
              </span>
            </div>
            <Link href="/settings" className="text-xs transition-colors" style={{ color: "#FFB800", fontFamily: "var(--font-dm-mono)" }}>
              Upgrade →
            </Link>
          </div>
        )}
        ...
        <style>{`input[type='range']::-webkit-slider-thumb { -webkit-appearance:none; width:16px; height:16px; background:${subjectColor}; border:2px solid #08080F; border-radius:0; cursor:pointer; }`}</style>
      </div>
    );
  }
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        <div className="text-center py-8">
          <div className="inline-flex items-center justify-center mb-6"
            style={{ width: 72, height: 72, border: `1px solid ${accuracy >= 70 ? "#C8FF0050" : "#FF3D5750"}`,
          ...
          <h1 className="text-2xl font-black mb-2" style={{ fontFamily: "var(--font-syne)", color: "#F0F0FF" }}>
            {accuracy >= 80 ? "Excellent session!" : accuracy >= 60 ? "Good progress." : "Keep practicing."}
          </h1>
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            <div key={s.label} className="flex flex-col items-center py-5"
              ...
              <span className="text-2xl font-black leading-none mb-1" style={{ fontFamily: "var(--font-syne)", color: s.color }}>{s.val}</span>
              <span className="text-xs" style={{ color: "#3A3A5C", fontFamily: "var(--font-dm-mono)" }}>{s.label}</span>
            </div>
          ))}
        </div>
        <div className="flex gap-3">
          <button onClick={handleStart} className="flex-1 py-3.5 text-sm font-black tracking-widest transition-all"
            style={{ backgroundColor: subjectColor, color: "#08080F", fontFamily: "var(--font-syne)" }}>
            PRACTICE AGAIN →
          </button>
          <Link href="/dashboard" className="flex-1 py-3.5 text-sm font-black tracking-widest text-center transition-all"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2">
          <div className="flex items-center gap-3 mb-4">
            <div className="flex-1 h-1 overflow-hidden" style={{ backgroundColor: "#1E1E36" }}>
              <div className="h-full transition-all duration-500"
                style={{ width: `${(currentIndex / questions.length) * 100}%`, backgroundColor: subjectColor }} />
            </div>
            <span className="text-xs flex-shrink-0" style={{ color: "#3A3A5C", fontFamily: "var(--font-dm-mono)" }}>
              {currentIndex}/{questions.length}
            </span>
          </div>
          <QuestionCard key={currentQ.id + currentIndex} question={currentQ}
            onAnswer={handleAnswer} isPro={isPro}
            questionNumber={currentIndex + 1} total={questions.length} />
        </div>

        <div className="flex flex-col gap-4">
          <div className="p-4" style={{ backgroundColor: "#0F0F1A", border: "1px solid #1E1E36" }}>
            <p className="text-xs mb-3 tracking-widest uppercase" style={{ color: "#3A3A5C", fontFamily: "var(--font-dm-mono)" }}>Difficulty</p>
            <div className="flex items-center gap-3">
              <DifficultyPips level={difficulty} color={subjectColor} />
              <span className="text-xs" style={{ color: subjectColor, fontFamily: "var(--font-dm-mono)" }}>Level {difficulty}</span>
            </div>
            <p className="text-xs mt-2" style={{ color: "#3A3A5C", fontFamily: "var(--font-dm-mono)" }}>
          ...
          <div className="p-4" style={{ backgroundColor: "#0F0F1A", border: "1px solid #1E1E36" }}>
            <p className="text-xs mb-3 tracking-widest uppercase" style={{ color: "#3A3A5C", fontFamily: "var(--font-dm-mono)" }}>Session Score</p>
            <div className="text-3xl font-black leading-none mb-1"
              style={{ fontFamily: "var(--font-syne)", color: accuracy >= 70 ? "#C8FF00" : "#FF3D57" }}>
              {results.length > 0 ? `${accuracy}%` : "—"}
            </div>
            <p className="text-xs" style={{ color: "#3A3A5C", fontFamily: "var(--font-dm-mono)" }}>{correctCount}/{results.length} correct</p>
          </div>

          {results.length > 0 && (
            <div className="p-4" style={{ backgroundColor: "#0F0F1A", border: "1px solid #1E1E36" }}>
              <p className="text-xs mb-3 tracking-widest uppercase" style={{ color: "#3A3A5C", fontFamily: "var(--font-dm-mono)" }}>History</p>
              <div className="flex flex-wrap gap-1.5">
                {results.map((r, i) => (
                  <div key={i} className="w-6 h-6 flex items-center justify-center text-xs"
                    style={{ backgroundColor: r.correct ? "#C8FF0015" : "#FF3D5715",
          ...
          {!isPro && (
            <div className="p-3" style={{ backgroundColor: "#FFB80008", border: "1px solid #FFB80025" }}>
              <p className="text-xs" style={{ color: "#FFB800", fontFamily: "var(--font-dm-mono)" }}>
                {results.length}/{FREE_DAILY_LIMIT} free questions used
              </p>
            </div>
          )}

          <button onClick={() => setFinished(true)} className="w-full py-2.5 text-xs tracking-widest transition-all"
            style={{ border: "1px solid #1E1E36", color: "#3A3A5C", fontFamily: "var(--font-dm-mono)" }}
