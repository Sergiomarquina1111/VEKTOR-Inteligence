...
_dkg_cache: dict[str, dict] = {}
_dkg_nx_cache: dict[str, nx.DiGraph] = {}
...
def _normalise_key(stem: str) -> str:
    """
    Normalise a filename stem to a clean subject key.
    physics_dkg → physics
    mathematics_linear_algebra → mathematics_linear_algebra  (kept as-is)
    mathematics_calculus → mathematics_calculus              (kept as-is)
    """
    # Strip trailing _dkg suffix if present
    if stem.endswith("_dkg"):
        stem = stem[:-4]
    return stem
...
def load_all_dkgs():
    ...
    for json_file in sorted(_DKG_DIR.glob("*.json")):
        key = _normalise_key(json_file.stem)
        try:
...
def _build_nx_graph(dkg_data: dict) -> nx.DiGraph:
    ...
    Edges only have source, target, weight — no relation field.
    """
    G = nx.DiGraph()
    for node in dkg_data.get("nodes", []):
        G.add_node(
            node["id"],
            label=node["label"],
            description=node.get("description", ""),
            tier=node.get("tier", "foundational"),
            aliases=node.get("aliases", []),
            prerequisites=node.get("prerequisites", []),
            successors=node.get("successors", []),
        )
    for edge in dkg_data.get("edges", []):
        G.add_edge(
            edge["source"],
            edge["target"],
...
def _select_dkg_key(subject: str) -> Optional[str]:
    """
    Select the best DKG cache key for a given subject string.

    Priority order:
      1. Exact match              physics → physics
      2. Starts-with match        mathematics → mathematics_calculus (first found)
      3. None — log warning
    """
    subject = subject.lower().strip().replace(" ", "_")

    # Remove any _dkg suffix the caller might pass
    subject = _normalise_key(subject)
    ...
    logger.warning(
        "No DKG found for subject '%s'. Loaded: %s",
        subject, list(_dkg_cache.keys())
    )
    return None
...
def get_dkg_for_subject(subject: str) -> Optional[tuple[dict, nx.DiGraph]]:
    ...
    Never raises KeyError — returns None and logs a warning instead.
    """
...
def build_skg(triples: list[dict]) -> nx.DiGraph:
    ...
    Nodes are student concept strings; edges carry the relation label.
    """
    G = nx.DiGraph()
    for triple in triples:
        src  = triple["subject"]
        tgt  = triple["object"]
        rel  = triple.get("relation", "leads_to")
...
def match_skg_to_dkg(skg: nx.DiGraph, subject: str) -> dict[str, Optional[str]]:
    ...
    Returns dict mapping skg_node_label → dkg_node_id (or None if below threshold).
    """
    if not skg.nodes:
        return {}
    ...
    similarity_matrix = np.dot(skg_embs, dkg_embs.T)
    ...
    for i, skg_label in enumerate(skg_labels):
        ...
        if best_score >= _SIMILARITY_THRESHOLD:
            ...
            logger.info(
                "MATCH  '%s' → '%s' (score=%.3f)",
                skg_label, node_ids[best_idx], best_score
            )
        else:
            mapping[skg_label] = None
            logger.info(
                "MISS   '%s' best='%s' score=%.3f (threshold=%.2f)",
                skg_label, node_ids[best_idx], best_score, _SIMILARITY_THRESHOLD
            )
...
def get_similarity_scores(skg: nx.DiGraph, subject: str) -> dict[str, float]:
    """Return the best similarity score per SKG node (for response metadata)."""
